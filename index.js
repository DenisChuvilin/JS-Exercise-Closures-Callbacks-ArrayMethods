...
function processSum(numberList, cb) {
  return cb(numberList.reduce((accumulator, c) => accumulator + c, 0));
}
/**
...
function processContains(item, list, callback) {
  const bool = list.includes(item) ? true : false;
...
function processDuplicateFree(list, callback) {
  ...
  const nodupes = [];
  callback(
    //push only one kind of an element into array
    list.forEach(char => {
      if (!nodupes.includes(char)) nodupes.push(char);
    })
  );
  //return newarray
  return nodupes;
